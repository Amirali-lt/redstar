...
export function handleTelegramApiError(error, context) {
    ...
    if (error.code === 'ETELEGRAM') {
        ...
        if (
            ...
            console.warn(
                `[chatLogic:handleTelegramApiError] Blocked/Kicked by user/group in ${context}: ${description}`
            );
            return true;
        }
    }
    ...
    console.error(
        `[chatLogic:handleTelegramApiError] Unhandled error in ${context}:`,
        error.message
    );
...
export async function handleChatLogic(bot, msg, userPrompt, deps) {
    console.log(
        `[chatLogic:handleChatLogic] START (User: ${msg.from.id}, Chat: ${msg.chat.id})`
    );
    ...
    if (msg.chat.type !== 'private' && !cache.authorizedChats.has(chatId)) {
        console.log(
            `[chatLogic:handleChatLogic] END - Chat ID ${chatId} not authorized.`
        );
        return;
    }

    if (activeUsers.has(userId)) {
        const cooldownMessage = await db.getText(
            'chat_cooldown',
            "صبر کن رفیق! دارم به سوال قبلی خودت فکر می‌کنم. یکی یکی!"
        );
        ...
        sendMessageSafe(
            ...
            handleTelegramApiError(
                err,
                'handleChatLogic - active user message'
            )
        );
    ...
    const limitCheck = await limitManager.checkUserLimit(
        userId,
        appConfig
    );
    ...
    if (limitCheck.requiresPrivateChatCheck) {
        ...
        if (isBlocked) {
            const requiredMessage = await db.getText(
                'private_chat_required_message',
                "لطفاً برای ادامه چت، ربات را در چت خصوصی استارت/آنبلاک کنید."
            );

            const requiredButton =
                await limitManager.createPrivateChatRequiredButton();
            ...
            return sendMessageSafe(
                ...
                handleTelegramApiError(
                    err,
                    'handleChatLogic - private chat required message'
                )
            );
        }
    }
    ...
    if (!limitCheck.allowed) {
        const hasNotified = await db.hasReceivedLimitMessage(
            userId,
            'day'
        );

        if (!hasNotified) {
            const limitMessageTemplate = await db.getText(
                'limit_message_day',
                "محدودیت روزانه..."
            );

            const helpButton =
                await limitManager.createAPIRequestButton();

            await db.markLimitMessageSent(userId, 'day');

            console.log(
                `[chatLogic:handleChatLogic] END - User ${userId} hit daily limit (Sending notification).`
            );
        ...
        console.log(
            `[chatLogic:handleChatLogic] END - User ${userId} hit daily limit (Already notified).`
        );
    ...
    const isPremium =
        limitCheck.isPremium || limitCheck.isOwner;

    activeUsers.add(userId);

    let placeholder;

    const thinkingText = await db.getText(
        'error_thinking',
        `${characterf.firstname} داره فکر می‌کنه...`
    );
    ...
    try {
        ...
        let history = await db.getDailyConversation(
            chatId,
            userId
        );
        ...
        let finalSystemInstruction =
            appPrompts.getSystemInstruction();
...
${memory.summary}`;
        ...
        const reinjectPromptEvery =
            appConfig.reinjectPromptEvery ||
            REINFORCE_EVERY_N_TURNS;

        const needsPeriodicReinforcement =
            conversationTurns > 0 &&
            conversationTurns % reinjectPromptEvery === 0;

        const needsReinforcement =
            !reinforcedUsersThisSession.has(userId) ||
            history.length === 0 ||
            needsPeriodicReinforcement;

        if (needsReinforcement) {
            historyForAPI.unshift(
                outputFilter.modelAck
            );

            historyForAPI.unshift(
                outputFilter.reinforcementPrompt
            );
        ...
        if (result.filtered) {
            characterResponse = await db.getText(
                'error_general',
                "محتوا فیلتر شد."
            );
        } else {
            characterResponse = result.text;
        }

        if (result.metadata?.totalTokenCount) {
            const tokenCount =
                result.metadata.totalTokenCount;

            console.log(
                `[chatLogic:handleChatLogic] Token usage: ${tokenCount}`
            );

            // Note: Actual key ID tracking would require apiService
            // to return it or handle increment internally.
            // Assuming key ID is abstracted away,
            // global stats can still be tracked if db structure supports it.
        }
        ...
        historyToSave.push(
            ...
            },
            {
                ...
                parts: [
                    {
                        text: characterResponse.substring(0, 4000)
                    }
                ]
            }
        );

        const safeTurns = Math.min(
            appConfig.maxHistoryTurns || 10,
            MAX_SAFE_TURNS
        );
        ...
        while (
            historyToSave.length > maxFinalLength ||
            calculateHistorySize(historyToSave) > MAX_HISTORY_BYTES
        ) {
            historyToSave.splice(0, 2);
        }

        await db.saveDailyConversation(
            chatId,
            userId,
            historyToSave
        );
        ...
        const globalButton =
            cache.globalSettings.global_button;

        const isChatSpecial =
            await db.isSpecialChat(chatId);

        const buttonEnabled =
            cache.globalSettings.global_button_enabled;
        ...
        const hasValidButton =
            ...
            globalButton &&
            !isChatSpecial &&
            typeof globalButton.text === 'string' &&
            globalButton.text.trim().length > 0 &&
            typeof globalButton.url === 'string' &&
            globalButton.url.trim().length > 0;
        ...
        try {
            ...
            if (
                ...
                telegramError.response.body.description.includes(
                ...
                await bot.editMessageText(
                    ...
                    handleTelegramApiError(
                        err,
                        'handleChatLogic - final response fallback'
                    )
                );
            } else {
                throw telegramError;
        ...
        console.error(
            `[chatLogic:handleChatLogic] Error:`,
            error.message
        );

        const errorMessage = await db.getText(
            'error_general',
            "خطای عمومی"
        );
